<template>
  <v-container fluid>
    ...
    <v-dialog
      scrollable
      max-width="60%"
      :persistent="editingEmployee || creatingEmployee"
    ...
    >
      <v-card>
        ...
        <v-card-text>
          ...
          <v-card v-if="!creatingEmployee" outlined class="mb-2">
            <v-list-item three-line>
              <v-list-item-content>
                ...
                <span>{{
                  editedItem.category.name ? editedItem.category.name : ""
                }}</span
                ><br />
                <span>{{
                  editedItem.temporaryAddress ? editedItem.temporaryAddress : ""
                }}</span
                ><br />
                  ...
                  <v-tooltip bottom>
                    <template v-slot:activator="{ attrs, on }">
                      <v-btn
                        ...
                        @click="step = (parseInt(step) + 1).toString()"
                        :disabled="
                          ($v.editedItem.$invalid && !editingEmployee) ||
                          step === '3'
                        "
                      >
                  ...
                  <v-row dense>
                    ...
                    <v-col cols="6">
                      <v-text-field
                        ...
                        :error-messages="
                          getErrors('staffName', $v.editedItem.staffName)
                        "
                    ...
                    <v-col cols="6">
                      ...
                      >
                        <v-row no-gutters>
                          <v-col cols="12">
                            <span style="font-size: 12px"
                              >Full Time Join Date (BS)</span
                            >
                          </v-col>
                          <v-col cols="12">
                            <VNepaliDatePicker
                              classValue="nepali-datepicker"
                              v-model="editedItem.fullTimeJoinDate"
                              calenderType="Nepali"
                              placeholder="yyyy-mm-dd"
                              format="YYYY-MM-DD"
                            />
                    ...
                    <v-col cols="6">
                      ...
                      >
                        <v-row no-gutters>
                          <v-col cols="12">
                            <span style="font-size: 12px"
                              >Part Time Join Date (BS)</span
                            >
                          </v-col>
                          <v-col cols="12">
                            <VNepaliDatePicker
                              classValue="nepali-datepicker"
                              v-model="editedItem.partTimeJoinDate"
                              calenderType="Nepali"
                              placeholder="yyyy-mm-dd"
                              format="YYYY-MM-DD"
                            />
                    ...
                    <v-col cols="6">
                      <v-autocomplete
                        ...
                        return-object
                        :error-messages="
                          getErrors('category', $v.editedItem.category)
                        "
                            ...
                            <v-container fluid>
                              <v-row no-gutters>
                                ...
                                >
                                  <v-checkbox
                                    @change="
                                      setBasicHours(editedItem.assignedHours)
                                    "
                                    :readonly="
                    ...
                    <v-col cols="4">
                      <v-autocomplete
                        prepend-inner-icon="fa-solid fa-venus-mars"
                        filled
                        dense
                        :items="genderItems"
                    ...
                    <v-col cols="4">
                      <v-text-field
                        ...
                        :error-messages="
                          getErrors('basicHours', $v.editedItem.basicHours)
                        "
              ...
              <v-stepper-content step="2">
                <v-container fluid>
                  <v-row dense>
                    <v-col cols="12">
                      <v-autocomplete
                        prepend-inner-icon="fa-solid fa-university"
                        filled
                        dense
                        :items="allActiveBanks"
                        :readonly="!editingEmployee && !creatingEmployee"
              ...
              <v-stepper-content step="3">
                <v-container fluid>
                  <v-row dense>
                    <v-col cols="6">
                      <v-autocomplete
                        prepend-inner-icon="fa-solid fa-heartbeat"
                        filled
                        dense
                        :items="allBloodGroups"
                    ...
                    <v-col cols="6">
                      <v-text-field
                        ...
                        v-model="
                          editedItem.employeeEmergencyDetails.contactName
                        "
                      ></v-text-field>
                    </v-col>
                    <v-col cols="6">
                      <v-text-field
                        ...
                        v-model="
                          editedItem.employeeEmergencyDetails.contactNumber
                        "
                      ></v-text-field>
                    </v-col>
                    <v-col cols="6">
        ...
        <v-card-actions v-if="editingEmployee || creatingEmployee">
          ...
          <v-btn
            color="primary"
            :disabled="
              (creatingEmployee && step !== '3') || $v.editedItem.$invalid
            "
    ...
    <v-dialog scrollable max-width="50%" v-model="mappedClientsDialog">
      <v-card>
        <v-toolbar dark flat class="mb-6" color="primary">
          <v-toolbar-title
            >{{ editedTableItem.staffName }}'s Clients</v-toolbar-title
          >
          <v-spacer></v-spacer>
          <v-toolbar-items>
            <v-btn
          ...
          <v-card outlined class="mb-2">
            <v-list-item three-line>
              <v-list-item-content>
                ...
                <span>{{
                  editedTableItem.categoryName
                    ? editedTableItem.categoryName
                    : ""
                }}</span
                ><br />
                <span>{{
                  editedTableItem.email ? editedTableItem.email : ""
                }}</span>
                <span>{{
                  editedTableItem.phone ? editedTableItem.phone : ""
                }}</span>
              </v-list-item-content>
              <v-chip
                :color="filterMenuItems[0].color"
                ...
                <v-row dense>
                  <v-col cols="7">
                    <v-autocomplete
                      outlined
                      dense
                      :items="allActiveClientsFiltered"
                      v-model="clientMappings.newMapping.client"
                      label="Client"
                      item-text="siteName"
                      return-object
                      :error-messages="
                        getErrors('client', $v.clientMappings.newMapping.client)
                      "
                  ...
                  <v-col cols="2">
                    <v-tooltip bottom>
                      <template v-slot:activator="{ attrs, on }">
                        <v-btn
                          :disabled="$v.clientMappings.newMapping.$invalid"
                ...
                <template v-slot:item.actions="{ item }">
                  <v-tooltip bottom>
                    <template v-slot:activator="{ attrs, on }">
                      <v-btn
                        color="error"
                        @click.stop.prevent="deleteClientMapping(item)"
                        v-bind="attrs"
                        v-on="on"
                        fab
                        icon
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" @click="updateClientMappings()"> Save </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <v-dialog scrollable max-width="50%" v-model="viewLeaveDetailsDialog">
      <v-card>
        <v-toolbar dark flat class="mb-6" color="primary">
          <v-toolbar-title
            >{{ editedTableItem.staffName }}'s Leave Details</v-toolbar-title
          >
          <v-spacer></v-spacer>
          <v-toolbar-items>
    ...
    <v-dialog scrollable max-width="50%" v-model="updateEmployeeStatusDialog">
      <v-card>
        <v-toolbar dark flat class="mb-6" color="primary">
          <v-toolbar-title
            >Update {{ editedTableItem.staffName }}'s Status</v-toolbar-title
          >
          <v-spacer></v-spacer>
          <v-toolbar-items>
            <v-btn icon @click="updateEmployeeStatusDialog = false">
              <v-icon>fa-solid fa-xmark</v-icon>
            </v-btn>
          </v-toolbar-items>
        </v-toolbar>
        <v-card-text>
          ...
          <v-container fluid>
            <v-row dense>
              <v-col cols="12">
                <v-radio-group v-model="employeeStatusItem.activeStatus" column>
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn color="error" @click="updateEmployeeStatusDialog = false">
            Cancel
          </v-btn>
          <v-btn color="primary" @click="changeEmployeeStatus()"> Save </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-container>
...
export default {
  ...
  computed: {
    ...
    annualLeaveDetails() {
      return this.validateLeave(
        true,
        this.editedTableItem.fullTimeJoinDate
          ? this.editedTableItem.fullTimeJoinDate
          : this.editedTableItem.partTimeJoinDate,
    ...
    resignPendingDataWithSn() {
      return this.resignPendingData.map((d, index) => ({
        ...d,
        sno: index + 1,
      }));
    },
  ...
  methods: {
    ...
    setBasicHours(assignedHours) {
      ...
      this.editedItem.basicHours =
        (assignedHours || 0) * (30 - numberOfWeekends * 4);
    },
    ...
    deleteItem(item) {
      ...
      this.$root
        .confirm(
          "Confirm Delete",
          "Are you sure you want to delete " + item.staffName + "?"
        )
        .then((confirm) => {
        ...
        .dispatch("api/makePostRequest", {
          ...
          data: {
            ...
            employeeDetails: {
              ...
              dearnessAllowance:
                temp.editedItem.employeeDetails.dearnessAllowance,
              otherAllowance: temp.editedItem.employeeDetails.otherAllowance,
              specialAllowance:
                temp.editedItem.employeeDetails.specialAllowance,
              direnessAllowance:
                temp.editedItem.employeeDetails.direnessAllowance,
              hardnessAllowance:
                temp.editedItem.employeeDetails.hardnessAllowance,
            ...
            employeeEmergencyDetails: {
              ...
              contactNumber:
                temp.editedItem.employeeEmergencyDetails.contactNumber,
    ...
    reactivateEmployee(item) {
      ...
      this.$root
        .confirm(
          `Confirm`,
          `Are you sure you want to reactivate ${item.staffName}'s ?`
        )
        .then((confirm) => {
          this.makePostRequest({
  ...
  */
    ...
    deactivateEmployee(item) {
      ...
      this.$root
        .confirm(
          "Confirm Delete",
          "Are you sure you want to deactivate " + item.staffName + "?"
        )
        .then((confirm) => {
          this.makePostRequest({
