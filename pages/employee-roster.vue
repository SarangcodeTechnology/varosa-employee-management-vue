        ...
        <v-row>
          ...
          <v-col cols="6">
            <v-autocomplete
              ...
              filled
              dense
              :items="allActiveEmployeeEnglish"
              v-model="selectedEmployee"
            ...
            <v-col cols="auto">
              <v-btn-toggle>
                ...
                <v-text-field
                  ...
                  readonly
                  :value="
                    selectedEmployee.useNepaliCalendar
                      ? selectedNepaliYear
                      : selectedEnglishYear
                  "
                ></v-text-field>
    ...
    <v-dialog scrollable max-width="50%" v-model="viewLeaveDetailsDialog">
      <v-card>
        <v-toolbar dark flat class="mb-6" color="primary">
          ...
          <v-spacer></v-spacer>
          <v-toolbar-items>
            <v-btn icon @click="viewLeaveDetailsDialog = false">
              <v-icon>fa-solid fa-xmark</v-icon>
            </v-btn>
          </v-toolbar-items>
        </v-toolbar>
...
export default {
  ...
  methods: {
        ...
        currentEmpData.forEach(function (empData) {
          ...
          dataHeaders.forEach(function (headerName) {
            ...
            function getValueOf(key) {
              ...
              if (key == "Client's Name") return empData.client.siteName;
              if (key == "Worked Hr")
    ...
    getCurrentYearName() {
      return this.selectedEmployee.useNepaliCalendar
    ...
    deleteItem(item) {
      ...
      this.$root
        .confirm(
          "Confirm Delete",
          "Are you sure you want to delete " + item.client.siteName + "?"
        )
        .then((confirm) => {
          this.makePostRequest({
            ...
            data: {
              ...
              month: temp.selectedEmployee.useNepaliCalendar
                ? temp.selectedNepaliMonth
                : temp.selectedEnglishMonth,
              year: temp.selectedEmployee.useNepaliCalendar
    ...
    previousMonth() {
      if (this.selectedEmployee.useNepaliCalendar) {
        if (this.selectedNepaliMonth > 1) {
          ...
          return this.getEmployee(this.selectedEmployee);
        } else throw new Error("Cannot find previous month data");
      } else {
    ...
    newHourLog(event, rosterByEmployeeIndex, dayIndex, undoLeave) {
      ...
      this.employeeData[rosterByEmployeeIndex].totalHours = this.getTotalHours(
        this.selectedEmployee,
        this.employeeData[rosterByEmployeeIndex].days,
        this.numberOfDaysInCurrentMonth,
        this.selectedEmployee.useNepaliCalendar,
        this.selectedEmployee.useNepaliCalendar
          ? this.selectedNepaliYear
          : this.selectedEnglishYear,
        this.selectedEmployee.useNepaliCalendar
      ...
      if (
        this.newEmployeeRosters.filter(
          (item) => item.clientId === roster.client.id && item.date === day.date
        ).length !== 0
      ) {
        this.newEmployeeRosters = this.newEmployeeRosters.filter(
          (item) =>
            !(
              item.clientId === roster.client.id &&
              item.date === day.date &&
              !item.undoLeave
            )
        );
      ...
      if (this.employeeData[rosterByEmployeeIndex].days[dayIndex].isLeave) {
        ...
        if (leaveDetails.validity) {
          if (leaveDetails.numberOfValidLeavesPreviousYear > 0) {
            this.employeeData[rosterByEmployeeIndex].days[
              dayIndex
            ].leaveFiscalYear = this.selectedEmployee.useNepaliCalendar
              ? this.selectedNepaliYear - 1
              : this.selectedEnglishYear - 1;
          } else {
            this.employeeData[rosterByEmployeeIndex].days[
              dayIndex
            ].leaveFiscalYear = this.selectedEmployee.useNepaliCalendar
    ...
    setHolidayBasicHours(event, rosterByEmployeeIndex, dayIndex) {
      ...
      const holidayBasicHours =
        this.employeeData[this.selectedPayload.rosterByEmployeeIndex].days[
    ...
    saveToSalarySheet() {
      ...
      const calendarData = {
        ...
        selectedMonth: temp.selectedEmployee.useNepaliCalendar
          ? temp.selectedNepaliMonth
          : temp.selectedEnglishMonth,
        selectedYear: temp.selectedEmployee.useNepaliCalendar
          ? temp.selectedNepaliYear
          : temp.selectedEnglishYear,
        ...
        .then((response) => {
          if (response.data.status === "OK") {
            ...
            if (temp.selectedEmployee.employeeDetails == null) {
              ...
              const salaryDetails = this.getSalaryDetailsObject(
  ...
  computed: {
    ...
    annualLeaveDetails() {
      return this.validateLeave(
        true,
        this.selectedEmployee.fullTimeJoinDate
          ? this.selectedEmployee.fullTimeJoinDate
      ...
      for (let i = 1; i <= temp.numberOfDaysInCurrentMonth; i++) {
        if (
          this.isHoliday({
            ...
            selectedMonth: this.selectedEmployee.useNepaliCalendar
              ? temp.selectedNepaliMonth
              : temp.selectedEnglishMonth,
          ...
          !this.isWeekend({
            useNepaliCalendar: this.selectedEmployee.useNepaliCalendar,
            selectedYear: this.selectedEmployee.useNepaliCalendar
              ? temp.selectedNepaliYear
              : temp.selectedEnglishYear,
            selectedMonth: this.selectedEmployee.useNepaliCalendar
              ? temp.selectedNepaliMonth
    ...
    allActiveEmployeeNepali() {
      return this.allActiveEmployee.filter(
