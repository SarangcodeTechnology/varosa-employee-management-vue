<template>
  <v-container fluid>
        ...
        <v-row justify="center">
          ...
          <v-col cols="auto">
            <v-select
              menu-props="auto"
              chips
              @input="getData(selectedFilterOption)"
              solo
              flat
              dense
            ...
            <div class="d-flex justify-content-center align-items-center">
              <v-tooltip bottom>
                <template v-slot:activator="{ attrs, on }">
                  <v-btn
                    @click.stop.prevent="editUser(item, false)"
                    v-bind="attrs"
                    v-on="on"
                    fab
                    icon
              ...
              <v-tooltip v-if="activeStatus" bottom>
                <template v-slot:activator="{ attrs, on }">
                  <v-btn
                    @click.stop.prevent="editUser(item, true)"
                    v-bind="attrs"
                    v-on="on"
                    fab
                    icon
              ...
              <v-tooltip v-if="activeStatus" bottom>
                <template v-slot:activator="{ attrs, on }">
                  <v-btn
                    @click.stop.prevent="getFunctionalitiesByUser(item, true)"
                    v-bind="attrs"
                    v-on="on"
                    fab
                    icon
              ...
              <v-tooltip v-if="!activeStatus" bottom>
                <template v-slot:activator="{ attrs, on }">
                  <v-btn
                    @click.stop.prevent="reactivateUser(item)"
                    v-bind="attrs"
                    v-on="on"
                    fab
                    icon
    ...
    <v-dialog
      scrollable
      max-width="50%"
      :persistent="editingUser || creatingUser"
      v-model="userDialog"
    >
      <v-card>
        ...
        <v-card-text>
          <v-card v-if="!creatingUser" outlined>
    ...
    <v-dialog scrollable max-width="50%" v-model="mappedClientsDialog">
      <v-card>
        <v-toolbar dark flat class="mb-6" color="primary">
          <v-toolbar-title
            >{{ editedItem.staffName }}'s Clients</v-toolbar-title
          >
          <v-spacer></v-spacer>
          <v-toolbar-items>
            <v-btn
          ...
          <v-card outlined class="mb-2">
            <v-list-item three-line>
              <v-list-item-content>
                ...
                <span>{{
                  editedItem.categoryName ? editedItem.categoryName : ""
                }}</span
                ><br />
                <span>{{ editedItem.email ? editedItem.email : "" }}</span>
                <span>{{ editedItem.phone ? editedItem.phone : "" }}</span>
    ...
    <v-dialog
      scrollable
      max-width="50%"
      :persistent="editingFunctionalities"
      v-model="functionalitiesDialog"
    >
      <v-card>
        <v-toolbar dark flat class="mb-6" color="primary">
        ...
        <v-card-text>
          ...
          <v-container fluid>
            <v-row>
              <v-col cols="6">
                <v-checkbox
                  v-for="(functionality, i) in allUserFunctionalities"
                  :key="i"
                  v-if="i < 6"
                  :label="functionality.name"
                  :value="functionality.id.toString()"
                  v-model="editedFunctionalities"
                  multiple
                >
                </v-checkbox>
              </v-col>
        ...
        <v-card-actions>
          ...
          <v-btn color="primary" @click="updateFunctionalities()"> Save </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-container>
