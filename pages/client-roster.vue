            ...
            <v-col cols="auto">
              <v-btn-toggle>
                ...
                <v-text-field
                  ...
                  readonly
                  :value="
                    selectedClient.useNepaliCalendar
                      ? selectedNepaliYear
                      : selectedEnglishYear
                  "
                ></v-text-field>
                  ...
                  <v-menu close-on-content-click offset-y>
                    ...
                    >
                          ...
                          >{{
                            (
                              !!rosterByEmployees[index].days[j]
                                ? rosterByEmployees[index].days[j].isLeave
                                : false
                            )
                              ? "Undo Leave"
                              : "Leave"
                      ...
                      >
                        <v-list-item-title
                          >{{
                            (
                              !!rosterByEmployees[index].days[j]
                                ? rosterByEmployees[index].days[j].isSick
                                : false
                            )
                              ? "Undo Sick Leave"
                              : "Sick Leave"
                      ...
                      >
                        <v-list-item-title
                          >{{
                            (
                              !!rosterByEmployees[index].days[j]
                                ? rosterByEmployees[index].days[j].isAbsent
                                : false
                            )
                              ? "Undo Absent"
                              : "Absent"
                      ...
                      <v-list-item
                        v-if="
                          !!item.days
                            ? !isRelieverEmployee(rosterByEmployees[index].days)
                            : false
                        "
                        @click="addReliever(item.days[j].workedHours, index, j)"
                      >
                        <v-list-item-title>Add Reliever </v-list-item-title>
                      </v-list-item>
                      <v-list-item
                        v-if="!!item.days[j] ? item.days[j].isHoliday : false"
                        @click="
    ...
    <v-dialog scrollable max-width="40%" v-model="employeeModal">
      <v-card>
        <v-toolbar dark flat class="mb-6" color="primary">
          ...
          <v-spacer></v-spacer>
          <v-toolbar-items>
            <v-btn icon @click="employeeModal = false">
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn color="error" @click="employeeModal = false"> Cancel </v-btn>
          <v-btn color="primary" @click="addRelieverEmployee()"> Save </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <v-dialog scrollable max-width="60%" v-model="holidayModal">
      <v-card>
        <v-toolbar dark flat class="mb-6" color="primary">
          ...
          <v-spacer></v-spacer>
          <v-toolbar-items>
            <v-btn icon @click="holidayModal = false">
              <v-icon>fa-solid fa-xmark</v-icon>
            </v-btn>
          </v-toolbar-items>
        </v-toolbar>
        ...
        <v-card-actions>
          <v-spacer />
          <v-btn color="error" @click="holidayModal = false"> Cancel </v-btn>
          <v-btn color="primary" @click="saveHolidayBasicHours"> Save </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-container>
...
export default {
  ...
  methods: {
    markChecked() {
      ...
      console.log("this.rosterByEmployees", this.rosterByEmployees);
      return "done";
    },
    ...
    onPaginationChange(pagintaion) {
      ...
      this.rowsPerPage = pagintaion.itemsPerPage;
    ...
    getExportHeaders() {
      const currentEmpData = this.rosterByEmployees.length > 0 ? this.rosterByEmployees : this.rosterByEmployeesCache;
          ...
          .then((response) => {
            if (response.data.status === "OK") {
              ...
              for (const key in response.data.data) {
                const isReliever = this.isRelieverEmployee(
                ...
                const totalHours = temp.getTotalHours(
                  response.data.data[key][0].employee,
                  days,
                  temp.numberOfDaysInCurrentMonth,
                  item.useNepaliCalendar,
                  temp.selectedYear,
                ...
                roster.numberOfHolidays = roster.days.filter(
              ...
              temp.rosterByEmployeesCache = tempRoster ;
            }
    ...
    getListOfDaysInCurrentMonth(
      ...
      for (let i = 1; i <= numberOfDaysInCurrentMonth; i++) {
        const day = {
          ...
          isLeave: this.checkIsLeave({
            roster: employeeRosterData,
            date: i,
            useNepaliCalendar: useNepaliCalendar,
          }),
          isSick: this.checkIsSick({
            roster: employeeRosterData,
            date: i,
            useNepaliCalendar: useNepaliCalendar,
          }),
          isAbsent: this.checkIsAbsent({
            roster: employeeRosterData,
            date: i,
            useNepaliCalendar: useNepaliCalendar,
          }),
          isOvertime: this.checkIsOvertime({
            roster: employeeRosterData,
            date: i,
            useNepaliCalendar: useNepaliCalendar,
          }),
    ...
    getTotalHours(
      ...
      days.forEach((day) => {
        ...
        if (day.workedHours) {
          if (day.isHoliday) {
            if (day.workedHours > employee.assignedHours) {
              day.isOvertime = true;
              totalOvertimeHours =
                totalOvertimeHours + (day.workedHours - employee.assignedHours);
        ...
        if (day.holidayBasicHours > 0) {
          if (day.holidayBasicHours > employee.assignedHours) {
            ...
            totalOvertimeHours =
              totalOvertimeHours +
              (day.holidayBasicHours - employee.assignedHours);
            totalHolidayBasicHours =
              totalHolidayBasicHours + employee.assignedHours;
          } else {
            totalHolidayBasicHours =
              totalHolidayBasicHours + day.holidayBasicHours;
          }
        }
      ...
      totalWorkedHours = this.getExpectedBasicHours(
      ...
      totalBasicHours = this.getExpectedBasicHours(
        numberOfWorkingDaysInCurrentMonth,
        numberOfWorkedDays,
        employee.nonWorkingDays.length,
        totalBasicHours,
    ...
    newHourLog(event, rosterByEmployeeIndex, dayIndex, undoLeave) {
      ...
      this.rosterByEmployees[rosterByEmployeeIndex].days[dayIndex].workedHours =
        workedHours;

      // Extra overtime validation: Set overtime to false when hours less than assigned hours
      if (
        workedHours <=
        this.rosterByEmployees[rosterByEmployeeIndex].employee.assignedHours
      ) {
      ...
      this.rosterByEmployees[rosterByEmployeeIndex].totalHours =
        this.getTotalHours(
          roster.employee,
          this.rosterByEmployees[rosterByEmployeeIndex].days,
      ...
      if (this.selectedClient.useNepaliCalendar) {
        ...
        englishDay = day.date;
        englishFullDate = `${englishYear}-${englishMonth}-${englishDay}`;
        const tempNepaliDate = NepaliDate().getBS(
          englishYear,
          englishMonth,
          englishDay
        );
      ...
      if (
        this.newEmployeeRosters.filter(
          (item) =>
            item.employeeId === roster.employee.id && item.date === day.date
        ).length !== 0
      ) {
        this.newEmployeeRosters = this.newEmployeeRosters.filter(
          (item) =>
            !(
              item.employeeId === roster.employee.id &&
              item.date === day.date &&
              !item.undoLeave
            )
        );
    ...
    setLeave(event, rosterByEmployeeIndex, dayIndex) {
      ...
      let undoLeave;
      if (
        this.rosterByEmployees[rosterByEmployeeIndex].days[dayIndex].isLeave
      ) {
        this.rosterByEmployees[rosterByEmployeeIndex].days[
          dayIndex
        ].isLeave = false;
        ...
        const annualLeavesForPreviousFiscalYear =
          this.annualLeavesPreviousFiscalYear;
        const paidLeavesForPreviousFiscalYear =
          this.paidLeavesPreviousFiscalYear;

        const numberOfLeavesTakenThatAreNotSavedYet =
          this.newEmployeeRosters.filter(
            (item) => item.employeeId === employeeAtHand.id && item.isLeave
          ).length;
        const leaveDetails = this.validateLeave(
          true,
          employeeAtHand.fullTimeJoinDate
            ? employeeAtHand.fullTimeJoinDate
            : employeeAtHand.partTimeJoinDate,
          this.selectedNepaliMonth,
          this.selectedNepaliYear,
          annualLeavesForThisFiscalYear,
          annualLeavesForPreviousFiscalYear,
          paidLeavesForPreviousFiscalYear,
        ...
        if (leaveDetails.validity) {
          if (leaveDetails.numberOfValidLeavesPreviousYear > 0) {
            this.rosterByEmployees[rosterByEmployeeIndex].days[
              dayIndex
            ].leaveFiscalYear = this.selectedYear - 1;
          } else {
            this.rosterByEmployees[rosterByEmployeeIndex].days[
              dayIndex
    ...
    setSickLeave(event, rosterByEmployeeIndex, dayIndex) {
      ...
      if (this.rosterByEmployees[rosterByEmployeeIndex].days[dayIndex].isSick) {
        ...
        const sickLeavesForPreviousFiscalYear =
          this.sickLeavesPreviousFiscalYear;
        const paidLeavesForPreviousFiscalYear =
          this.paidLeavesPreviousFiscalYear;

        const numberOfLeavesTakenThatAreNotSavedYet =
          this.newEmployeeRosters.filter(
            (item) => item.employeeId === employeeAtHand.id && item.isSick
          ).length;
        const leaveDetails = this.validateLeave(
          false,
          employeeAtHand.fullTimeJoinDate
          ...
          this.selectedNepaliMonth,
          this.selectedNepaliYear,
          sickLeavesForThisFiscalYear,
          sickLeavesForPreviousFiscalYear,
          paidLeavesForPreviousFiscalYear,
          numberOfLeavesTakenThatAreNotSavedYet
        );
        if (leaveDetails.validity) {
    ...
    setAbsent(event, rosterByEmployeeIndex, dayIndex) {
      let undoLeave;
      if (
        this.rosterByEmployees[rosterByEmployeeIndex].days[dayIndex].isAbsent
      ) {
        this.rosterByEmployees[rosterByEmployeeIndex].days[
          dayIndex
        ].isAbsent = false;
    ...
    setHolidayBasicHours(event, rosterByEmployeeIndex, dayIndex) {
      ...
      const holidayBasicHours =
        this.rosterByEmployees[this.selectedPayload.rosterByEmployeeIndex].days[
          this.selectedPayload.dayIndex
        ].holidayBasicHours;
    ...
    validateLeave(
      ...
      if (employeeJoinDate == null) {
        return {
          numberOfTakenLeavesThisYear: 0,
          numberOfTakenLeavesPreviousYear: 0,
          numberOfValidLeavesThisYear: 0,
      ...
      const numberOfMonthsSinceJoinedToLastShrawan =
        this.getNumberOfMonthsSinceJoined(
          3,
          employeeJoinMonth,
          selectedNepaliYear,
          employeeJoinYear
      ...
      const numberOfWorkingMonthsInSelectedFiscalYear =
        numberOfMonthsSinceJoined > 12
          ? this.getNumberOfMonthsSinceLastShrawan(selectedNepaliMonth) - 1
          : numberOfMonthsSinceJoined - 1;
      const numberOfWorkingMonthsInPreviousFiscalYear =
        numberOfMonthsSinceJoinedToLastShrawan > 12
          ? 12
          : numberOfMonthsSinceJoinedToLastShrawan;
      ...
      if (isAnnualLeave) {
        numberOfValidLeavesThisYear =
          numberOfWorkingMonthsInSelectedFiscalYear * 1.5;
        numberOfValidLeavesPreviousYear =
          numberOfWorkingMonthsInPreviousFiscalYear * 1.5;
      } else {
        numberOfValidLeavesThisYear =
          numberOfWorkingMonthsInSelectedFiscalYear * 1;
      }
      let numberOfTakenLeavesThisYear = 0;
      if (isAnnualLeave) {
        numberOfTakenLeavesThisYear =
          takenLeavesThisFiscalYear.length +
          numberOfLeavesTakenThatAreNotSavedYet;
      } else {
        numberOfTakenLeavesThisYear =
          takenLeavesThisFiscalYear.length +
      ...
      let numberOfTakenLeavesPreviousYear =
        takenLeavesPreviousFiscalYear.length;
      let numberOfPaidOutLeaves = paidLeavesPreviousFiscalYear
        ? paidLeavesPreviousFiscalYear.length
        : 0;
      let validity = false;
      ...
      if (
        numberOfValidLeavesThisYear +
          numberOfValidLeavesPreviousYear -
          numberOfTakenLeavesThisYear -
          numberOfTakenLeavesPreviousYear -
          numberOfPaidOutLeaves >
        0
      ) {
        validity = true;
      }
    ...
    getNumberOfMonthsSinceJoined(
      ...
      const numberOfMonths =
        selectedNepaliMonth -
        employeeJoinMonth +
        (selectedNepaliYear - employeeJoinYear) * 12;
    ...
    saveHolidayBasicHours() {
      this.rosterByEmployees[this.selectedPayload.rosterByEmployeeIndex].days[
        this.selectedPayload.dayIndex
      ].holidayBasicHours = this.holidayBasicHours;
      this.newHourLog(
        this.selectedPayload.event,
        this.selectedPayload.rosterByEmployeeIndex,
        this.selectedPayload.dayIndex
      );
    ...
    saveToSalarySheet() {
      ...
      for (const roster of this.rosterByEmployees) {
        calendarData[roster.employee.id] = {
          ...
          selectedMonth: temp.selectedClient.useNepaliCalendar
            ? temp.selectedNepaliMonth
            : temp.selectedEnglishMonth,
          selectedYear: temp.selectedClient.useNepaliCalendar
            ...
            for (const eachItem of listOfAggregates) {
              ...
              if (selectedEmployee.employeeDetails == null) {
                ...
                salaryDetailsList.push(
                  this.getSalaryDetailsObject(
                    selectedEmployee,
                    eachItem.aggregate,
                    temp.allActiveHoliday,
                    calendarData[selectedEmployee.id]
                  )
                );
              }
            }
    ...
    getSalaryDetailsObject(
      ...
      let temp = this;
      const employeeWeekends = temp.translateWeekendStringToNames(
        selectedClient.nonWorkingDays
      );
      ...
      const actualWorkedHours = temp.getActualWorkedHours(
        selectedClient,
        employeeRosterAggregate,
        calendarData
      );
      ...
      const totalOvertimeAllowance =
        selectedClient.employeeDetails.overtimeRate *
      ...
      const reductionSalary = temp.getReductionSalary(
        grossSalary,
        selectedClient,
        actualTotalHours,
        actualWorkedHours,
      ...
      const totalSalary = temp.getTotalSalary(
        selectedClient,
        reductionSalary,
        employeeRosterAggregate
      );
      ...
      const basicSalaryAfterReduction =
        selectedClient.employeeDetails.basicSalary -
        temp.getReductionBasicSalary(
          selectedClient,
          actualTotalHours,
          actualWorkedHours,
          expectedWorkHours,
      ...
      return {
        ...
        bankName: selectedClient.employeeDetails.bank
          ? selectedClient.employeeDetails.bank.name
          : "N/A",
        accountNumber: selectedClient.employeeDetails?.accountNumber
          ? selectedClient.employeeDetails?.accountNumber
          : "N/A",
        ssfNo: selectedClient.employeeDetails?.ssfNo
          ? selectedClient.employeeDetails?.ssfNo
          : "N/A",
        englishYear,
        englishMonth,
        nepaliYear,
        ...
        holidayAlowance:
          selectedClient.employeeDetails.holidayRate *
    ...
    getActualWorkedHours(
      ...
      return this.getExpectedBasicHours(
        ...
        selectedClient.nonWorkingDays.length,
        employeeRosterAggregate.totalHours,
        selectedClient.basicHours,
        selectedClient.assignedHours
      );
    },
    ...
    addRelieverEmployee() {
      const currentEmployeeIdsInRoster = this.rosterByEmployees.map(
      ...
      if (currentEmployeeIdsInRoster.includes(this.selectedEmployee.id)) {
        ...
        const roster = {
          ...
          days: this.getListOfDaysInCurrentMonth(
            ...
            true,
            this.rosterByEmployees[this.rightClickPayload.rosterByEmployeeIndex]
              .employee.id
          ),
        };
        roster.numberOfHolidays = roster.days.filter(
        ...
        this.newHourLog(
          { target: { value: 0 } },
          this.rosterByEmployees.length - 1,
          0,
          false
        );
        this.applyChanges();
  ...
  computed: {
    ...mapGetters("calendar", [
      ...
      "checkIsOvertime",
      "checkIsReliever",
      "getRelievedEmployeeId",
      "getHolidayBasicHoursPerDate",
    ]),
    ...mapGetters("leave", [
      "allLeaves",
    ...
    annualLeaveDetails() {
      return this.validateLeave(
        true,
        this.selectedClient.fullTimeJoinDate
          ? this.selectedClient.fullTimeJoinDate
          : this.selectedClient.partTimeJoinDate,
        this.selectedNepaliMonth,
        this.selectedNepaliYear,
        this.annualLeaves,
        this.annualLeavesPreviousFiscalYear,
    ...
    getHeaders() {
      ...
      headers.push({
        text: "Leave hours",
        value: "totalHours.totalLeaveHours",
      });
      headers.push({ text: "Sick hours", value: "totalHours.totalSickHours" });
      headers.push({
        text: "Absent days",
        value: "totalHours.totalAbsentDays",
      });
      headers.push({
        text: "Holiday Basic Hours",
        value: "totalHours.totalHolidayBasicHours",
      });
      headers.push({
        text: "Total Hours",
        value: "totalHours.totalBasicHours",
      });
      headers.push({
        text: "Holiday Hours",
        value: "totalHours.totalHolidayHours",
      });
      headers.push({
        text: "Overtime Hours",
        value: "totalHours.totalOvertimeHours",
      });
    ...
    numberOfWeekendsInCurrentMonth() {
      ...
      for (let i = 1; i <= temp.numberOfDaysInCurrentMonth; i++) {
        const day = this.selectedClient.useNepaliCalendar
          ? new NepaliDate(
              this.selectedYear,
        ...
        const weekendList = !!this.selectedClient.nonWorkingDays
          ? this.selectedClient.nonWorkingDays.split("")
          : [];
        if (weekendList.includes(day.toString())) {
          numberOfWeekends++;
        }
      ...
      for (let i = 1; i <= temp.numberOfDaysInCurrentMonth; i++) {
        if (
          this.isHoliday({
            ...
            selectedMonth: this.selectedClient.useNepaliCalendar
              ? temp.selectedNepaliMonth
              : temp.selectedEnglishMonth,
          ...
          !this.isWeekend({
            useNepaliCalendar: this.selectedClient.useNepaliCalendar,
            selectedYear: this.selectedClient.useNepaliCalendar
              ? temp.selectedNepaliYear
              : temp.selectedEnglishYear,
            selectedMonth: this.selectedClient.useNepaliCalendar
              ? temp.selectedNepaliMonth
    ...
    allActiveClientNepali() {
      return this.allActiveClient.filter(
        (item) => item.useNepaliCalendar === true
      );
    },
