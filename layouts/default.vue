  ...
  <v-app dark>
        ...
        >
          <template v-for="item in tableItems">
            <template v-if="item.listItems.length > 0">
              ...
              <v-subheader v-else-if="item.subheading">{{
                item.subheading
              }}</v-subheader>
            </template>
            <v-list nav>
              <template v-for="listItem in item.listItems">
                ...
                <v-list-item v-else :to="listItem.to" router exact>
                  <v-list-item-action class="mr-4">
                    <v-icon>{{ listItem.icon }}</v-icon>
                  </v-list-item-action>
                  <v-list-item-content>
    ...
    <template v-if="!isPrinting">
      ...
      >
        ...
        <v-tooltip bottom>
          <template v-slot:activator="{ on, attrs }">
            <v-btn icon v-bind="attrs" @click="goBack" v-on="on">
              <v-icon  >fas fa-arrow-left</v-icon>
              <!-- <template v-bind="isPrinting" v-if="isPrinting"><span>is printing</span></template> -->
              <!-- <template v-else="isPrinting"><span>not printing</span></template> -->
            </v-btn>
          </template>
          <span>Back</span>
        </v-tooltip>
        ...
        <v-tooltip bottom>
          <template v-slot:activator="{ on, attrs }">
            <v-btn icon v-bind="attrs" @click="goForward" v-on="on">
            ...
            <v-tooltip v-if="hover" bottom>
              <template v-slot:activator="{ on, attrs }">
                ...
                >
                  <v-icon
                    >fas fa-{{
                      `window-${clipped ? "minimize" : "maximize"}`
                    }}</v-icon
                  >
                </v-btn>
              </template>
            ...
            <v-list class="pa-0 ma-0" dense>
              <v-list-item href="https://varosaservices.com/" target="_blank">
                <v-list-item-icon
                  ><img
                    alt="bigyapan_logo"
                    src="/images/varosa-logo-final-favicon.png"
                /></v-list-item-icon>
              ...
              <v-list-item
                :to="'/change-password'"
                @click="userMenuModal = !userMenuModal"
              >
                <v-list-item-icon>
        ...
        <v-tooltip bottom>
          <template v-slot:activator="{ on, attrs }">
            <v-btn v-bind="attrs" v-on="on" to="/settings" icon>
...
import printConrolBus from "/eventBuses/printControlBus";
...
const defaultLayout =  {
  ...
  methods: {
    ...
    setPrinting(value = true) {
      this.isPrinting = value;
      printConrolBus.$emit("isPrinting", value);
      console.log("Printing value changed to: " + value);
    },
  },
  watch: {
  ...
  computed: {
    isPrinting() {
      return this.$store.state.printControl.isPrinting;
    },
  ...
  created(){
    console.log(this.$store.state.printControl.isPrinting)
    printConrolBus.$on("isPrinting", (value) => {
      this.isPrinting = value;
      console.log("New event received in default layout: " + value) ;
    })
  }
};
